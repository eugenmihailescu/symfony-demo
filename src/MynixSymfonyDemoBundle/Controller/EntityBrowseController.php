...
use MynixSymfonyDemoBundle\Controller\GenericController;
...
final class EntityBrowseController extends GenericController {
	...
	public function browseEntityAction($entity, $page, $limit) {
		...
		$post_repository = $this->getDoctrine ()->getRepository ( 'MynixSymfonyDemoBundle:' . $entity );
		
		$em = $this->get ( 'doctrine.orm.entity_manager' );
		
		$persister = $em->getUnitOfWork ()->getEntityPersister ( 'MynixSymfonyDemoBundle:' . $entity );
		...
		if ($annotation) {
			...
			$html = $templating->render ( 'MynixSymfonyDemoBundle:entity:browse.html.twig', array (
					'pk' => $annotation->pk,
					'columns' => $columns,
